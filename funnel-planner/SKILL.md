---
name: funnel-planner
description: "This skill should be invoked when the user needs to set numeric business targets, model a funnel, or validate whether the math works for a business goal. Triggers include 'set targets', 'plan a funnel', 'create OKRs', 'what business metrics should I track', 'how many users do I need to hit [goal]', 'will the numbers work', 'what conversion rate do I need', 'forecast', 'projections', 'scenario modeling', 'unit economics', 'CAC', 'LTV', 'pricing model', 'path to profitability', or 'break-even' -- even if they just say 'can we hit $X', state a revenue target and need the funnel math worked backward, or have prioritized initiatives that need numeric targets. Not for mapping activities to existing KPIs (use attribution), defining experiment success metrics or sample sizes (use experiment), or diagnosing why metrics are declining (use problem-analysis)."
license: MIT
metadata:
  author: hungv47
  version: "3.2.0"
