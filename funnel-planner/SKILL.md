...
If upstream artifacts' `date` fields are older than 30 days, recommend re-running upstream skills before proceeding — stale baselines produce unreliable targets. Run `/navigate status` first for a single-pass freshness report across all upstream artifacts.
